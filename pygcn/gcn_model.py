...
from pygcn.gcn_layer import GraphConvolution
from pygcn.gcn_layer import RGCN
from pygcn.gcn_layer import Attention
import torch
...
class GCN(nn.Module):
    ...
    def get_embedding(
        ...
        cellular_embedding = graph_feature_dict['cellular']
        print("保存embedding字典")
        with open(DefaultConfig.road_embedding_path, mode='wb') as f:
            pickle.dump(road_embedding, f)
        with open(DefaultConfig.cell_embedding_path, mode='wb') as f:
            pickle.dump(cellular_embedding, f)
