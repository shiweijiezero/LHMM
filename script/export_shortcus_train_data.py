...
def export_networkx_obj():
    ...
    for line in tqdm(match_result_lines):
        ...
        for i in range(len(filtered_traj_mee_lst)):
            ...
            skip_one_cellular_id = -1
            skip_two_cellular_id = -1
            if (i >= 2):
                skip_one_mee_lng, skip_one_mee_lat = filtered_traj_mee_lst[i - 2].split()
                skip_one_mee_pos = (float(skip_one_mee_lng), float(skip_one_mee_lat))
                skip_one_cellular_id = pos2dgl_cellular_id[skip_one_mee_pos]
            else:
                continue
            if (i >= 3):
                skip_two_mee_lng, skip_two_mee_lat = filtered_traj_mee_lst[i - 3].split()
                skip_two_mee_pos = (float(skip_two_mee_lng), float(skip_two_mee_lat))
                skip_two_cellular_id = pos2dgl_cellular_id[skip_two_mee_pos]
            ...
            negetive_nids = []
            temp = list(set(temp1))
            if (skip_two_cellular_id != -1):
                temp.sort(
                    key=lambda x: MapData.get_distance(
                        (MapData.nxgraph.nodes[x]["lng"], MapData.nxgraph.nodes[x]["lat"]),
                        skip_two_mee_pos))
                positive_nids.extend(temp[:20])
                query_nids = MapData.query_ball_points(skip_two_mee_pos, 800)  # 其余800个点作为负样本
                negetive_nids.extend(query_nids)

            if (skip_one_cellular_id != -1):
                temp.sort(
                    key=lambda x: MapData.get_distance(
                        (MapData.nxgraph.nodes[x]["lng"], MapData.nxgraph.nodes[x]["lat"]),
                        skip_one_mee_pos))
                positive_nids.extend(temp[:20])
                query_nids = MapData.query_ball_points(skip_one_mee_pos, 800)  # 其余800个点作为负样本
                negetive_nids.extend(query_nids)
    ...
    print("保存shortcuts_train_data对象")
    with open(opt.shortcuts_train_data_path, mode='wb') as f:
